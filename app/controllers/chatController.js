...
let markChatAsSeen = (req, res) => {
  ...
  let validateParams = () => {
    return new Promise((resolve, reject) => {
      if (check.isEmpty(req.body.senderId) || check.isEmpty(req.body.userId)) {
      ...
      ChatModel.count(findQuery)
        .exec((err, result) => {
          if (err) {
            ...
            logger.error('not chats found','chatConteoller:unseen-chatsCount()',10)
            let apiResponse = response.generate(false, `no chats found`, 200, null)
            reject(apiResponse)
          }else {
...
let findallrooms=(req,res)=>{
    ChatRoomModel.find().select('-_id -__v')
    .exec((err,result)=>{
      if(err){
      res.send(response.generate(true,'some error occurred',500,null))
      logger.error('all rooms request failed','chatController;findallrooms()',10)
      }else if(check.isEmpty(result)){
      res.send(response.generate(true,'No rooms found',404,null))
      logger.error('all rooms request failed. Empty list','chatController;findallrooms()',10)
      }else{
      res.send(response.generate(false,'All Rooms Found',200,result))
      logger.info('all rooms request','chatController;findallrooms()',10)
      }
    })
}
/**
